...
class Point extends Chart {
    ...
    initData() {
        var that = this,
            xdis = this.W - this.padding * 2, //x轴实际长度
            ydis = this.H - this.padding * 2 - this.paddingTop,//y轴实际高度
            xl = this.xAxis.data.length,//x轴被分成多少段
            xs = xdis / xl,//x轴每段长度
        ...
        if (!this.series.length) return;//判断数据是否为空
        for (var i = 0; i < this.series.length; i++) {
            ...
            if (item.hide) continue;
            item.data.forEach(d => {
                arr.push(d.yVal);//每个点的y值
                numArr.push(d.num);//每个点值是多少
            });
        }

        numMax = Math.max.apply(null, numArr);//所有点的最大值
        numMin = Math.min.apply(null, numArr);//所有点的最小值

        // 计算数据在Y轴刻度信息（最小值，最大值，分几段，间隔多少）
